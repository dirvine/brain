...
impl Genome {
    ...
    pub fn get_last_node_id(&self) -> usize {
        self.neuron_genes().iter().last().unwrap().node_id + 1
    }
    //
    pub fn get_last_gene_innovnum(&self) {
        self.neuron_genes().iter().last().unwrap().innovation_num() + 1
    }
    ...
    pub fn mutate_link_weights(power: f64, rate: f64, mutator: f64) {
        unimplemented!();
        // 	std::vector<Gene*>::iterator curgene;
