...
impl Genome {
    ...
    pub fn last_innovation_number(&self) -> u32 {
    ...
    /// TODO
    pub fn mutate_link_weights(&mut self, power: f64, rate: f64, mutator: f64) {
        unimplemented!();
    }

    /// TODO
    pub fn randomise_traits(&mut self) {
        unimplemented!()
    }
    /// TODO
    ...
    //
    // // Add Gaussian noise to linkweights either GAUSSIAN or COLDGAUSSIAN (from
    // zero)
    ...
    // bool mutate_add_node(std::vector<Innovation*> &innovs,int &curnode_id,double
    // &curinnov);
    //
    // // Mutate the genome by adding a new link between 2 random NNodes
    // bool mutate_add_link(std::vector<Innovation*> &innovs,double &curinnov,int
    // tries);
    //
    ...
    // void
    //
    // Genome::Genome(int id, std::vector<Trait*> t, std::vector<NNode*> n,
    // std::vector<Gene*> g) {
    // 	genome_id=id;
    // 	traits=t;
    // 	nodes=n;
    ...
    // tempgene=new Gene((*curlink)->linktrait,
    // (*curlink)->weight,(*curlink)->in_node,(*curlink)->out_node,
    ...
    //
    // 			char argline[1024];
    // //strcpy(argline, NEAT::getUnits(curline, curwordnum, wordcount,
    // delimiters));
    //
    ...
    //
    //
    // Genome::Genome(int new_id,int i, int o, int n,int nmax, bool r, double
    // linkprob) {
    // 	int totalnodes;
    ...
    // outFile << "genomeend " << genome_id << std::endl << std::endl <<
    // std::endl;
    // 	//char tempbuf4[1024];
    ...
    // }
    //
    // pub fn get_last_node_id(&self) -> usize {
    //     self.network_nodes().iter().last().unwrap().node_id + 1
    // }
    // //
    // pub fn get_last_gene_innovnum(&self) {
    //     self.network_nodes().iter().last().unwrap().innovation_num() + 1
    // }
    ...
    // " << randfloat() << " " << power << " " << powermod << std::endl;
...
// }
//
// bool Genome::mutate_add_node(std::vector<Innovation*> &innovs,int
...
//
// std::vector<Innovation*>::iterator theinnov; //For finding a historical
// match
// 	bool done=false;
//
...
// newgene2=new
// Gene(traitptr,oldweight*0.3,newnode,out_node,false,curinnov+1,0);
// 				curinnov+=2.0;
// 			}
// 			else {
...
// innovs.push_back(new
// Innovation(in_node->node_id,out_node->node_id,curinnov-2.0,curinnov-1.0,
...
// 			if (thelink->is_recurrent) {
// newgene1=new
// Gene(traitptr,1.0,in_node,newnode,true,(*theinnov)->innovation_num1,0);
// newgene2=new
// Gene(traitptr,oldweight*0.3,newnode,out_node,false,
// (*theinnov)->innovation_num2,0);
// 			}
// 			else {
// newgene1=new
// Gene(traitptr,1.0,in_node,newnode,false,(*theinnov)->innovation_num1,0);
// newgene2=new
// Gene(traitptr,oldweight*0.3,newnode,out_node,false,
// (*theinnov)->innovation_num2,0);
// 			}
...
// //genes.push_back(newgene1);   //Old way to add genes- may result in genes
...
// }
//
// bool Genome::mutate_add_link(std::vector<Innovation*> &innovs,double
// &curinnov,int tries) {
//
...
// 				else {
// 					count=0;
// recurflag=phenotype->is_recur(nodep1->analogue,nodep2->analogue,count,
// thresh);
//
...
// 				if (phenotype==0) {
// //cout<<"ERROR: Attempt to add link to genome with no
// phenotype"<<std::endl;
// 					return false;
// 				}
//
...
// 				//Create the new gene
// newgene=new
// Gene(((thetrait[traitnum])),newweight,nodep1,nodep2,recurflag,curinnov,
// newweight);
//
// 				//Add the innovation
// innovs.push_back(new
// Innovation(nodep1->node_id,nodep2->node_id,curinnov,newweight,traitnum));
//
...
// 					//Create new gene
// newgene=new
// Gene(((thetrait[(*theinnov)->new_traitnum])),(*theinnov)->new_weight,nodep1,
// nodep2,recurflag,(*theinnov)->innovation_num1,0);
//
// 					done=true;
//
// 				}
...
// //genes.push_back(newgene);  //Old way - could result in out-of-order
// innovation numbers in rtNEAT
...
//
//
// void Genome::mutate_add_sensor(std::vector<Innovation*> &innovs,double
// &curinnov) {
//
...
// iter = sensors.erase(iter); //Does this work? remove by number from a
// vector?
// 		}
//
// 	}
...
// }
//
// Genome *Genome::mate_multipoint(Genome *g,int genomeid,double
// fitness1,double fitness2, bool interspec_flag) {
...
// 	NNode *new_inode;
// 	NNode *new_onode;
// std::vector<NNode*>::iterator curnode;  //For checking if NNodes exist
// already
...
// 	Gene *newgene;
//
// bool p1better; //Tells if the first genome (this one) has better fitness or
// not
//
// 	bool skip;
//
...
// 				else
// nodetraitnum=(((*curnode)->nodetrait)->trait_id)-(*(traits.
// begin()))->trait_id;
//
...
// 						//old buggy version:
// // if (!(onode->nodetrait))
// nodetraitnum=((*(traits.begin()))->trait_id);
// 						if (!(inode->nodetrait)) nodetraitnum=0;
// 						else
// nodetraitnum=((inode->nodetrait)->trait_id)-((*(traits.
// begin()))->trait_id);
//
...
// 						else
// nodetraitnum=((onode->nodetrait)->trait_id)-(*(traits.
// begin()))->trait_id;
//
...
// }
//
// Genome *Genome::mate_multipoint_avg(Genome *g,int genomeid,double
// fitness1,double fitness2,bool interspec_flag) {
// 	//The baby Genome will contain these new Traits, NNodes, and Genes
// 	std::vector<Trait*> newtraits;
// 	std::vector<NNode*> newnodes;
...
// ////with alpha=0.5, this will produce babies evenly in exploitation and
// exploration space
...
//
// 			//Add the Gene
// newgenes.push_back(new
// Gene(chosengene,newtraits[traitnum],new_inode,new_onode));
//
// 		}  //End of if (!skip)
//
// 		skip=false;
...
//
// //cout<<"COMPAT: size = "<<max_genome_size<<" disjoint =
// "<<num_disjoint<<" excess = "<<num_excess<<" diff = "<<mut_diff_total<<"
// TOTAL = "<<(disjoint_coeff*(num_disjoint/1.0)+excess_coeff*(num_excess/1.
// 0)+mutdiff_coeff*(mut_diff_total/num_matching))<<std::endl;
//
...
// Genome* NEAT::new_Genome_auto(int num_in,int num_out,int num_hidden,int
...
//
// 	oFile.close();
// }add_gene(std::vector<Gene*> &glist,Gene *g);
