...
/// Instead we call `rng.gen_range()` k-times. The drawn items could be the
/// same,
/// but the probability is very low if `n` is high compared to `k`.
#[inline]
pub fn tournament_selection<R: Rng, F>(rng: &mut R, better_than: &F, n: usize, k: usize) -> usize
...
pub fn tournament_selection_2<R: Rng, F>(rng: &mut R,
                                         better_than: &F,
                                         n: usize,
                                         k: usize,
                                         n_retries: usize)
    ...
    where F: Fn(usize, usize) -> bool
{
    let first = tournament_selection(rng, better_than, n, k);

    let mut retries = 0;
    loop {
        let second = tournament_selection(rng, better_than, n, k);
