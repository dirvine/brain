...
pub struct Species {
    ...
    expected_offspring: u32,
    novel: bool,
    obliterate: bool, // Allows killing off in competitive coevolution stagnation
    organisms: Vec<Organism>, // The organisms in the Species
    // std::vector<Organism*> reproduction_pool;  //The organisms for reproduction- NOT NEEDED
    age_of_last_improvement: u32, // If this is too long ago, the Species will goes extinct
    average_est: f64, // When playing real-time allows estimating average fitness
    params: Params,
}

impl Species {
    /// New species
    pub fn new(i: u32) -> Species {
        Species {
            id: i,
            age: 1,
            ave_fitness: 0.0,
    ...
    /// New novel species
    pub fn new_novel(i: u32, novel: bool) -> Species {
        Species {
            id: i,
            age: 1,
            ave_fitness: 0.0,
    ...
    /// Add an organism to this pecies
    pub fn add_organism(&mut self, org: Organism) {
        self.organisms.push(org);
    }
    /// Getter
    pub fn random_organism(&mut self) -> Option<&mut Organism> {
        let mut rng = thread_rng();
        rng.choose_mut(&mut self.organisms)
    }
    ...
    pub fn average_fitness(&self) -> f64 {
        ...
        let mut total = 0f64; //running total of fitnesses
...
// have been around long enough to count them in the fitness evaluation
        let mut num_orgs = 0f64; //counts number of orgs above the time_alive threshold
    ...
    /// Species total fitness
    pub fn total_fitness(&self) -> f64 {
        self.organisms.iter().fold(0f64, |sum, x| sum + x.fitness())
    }

    pub fn reproduce_one(&mut self,
                         generation: u32,
                         pop: Population,
                         sorted_species: Vec<Species>)
                         -> Organism {

        info!("In reproduce_one");
        debug_assert!(self.organisms.is_empty(),
                      "ERROR:  ATTEMPT TO REPRODUCE OUT OF EMPTY SPECIES");

        self.rank(); //Make sure organisms are ordered by rank
        let mut elig_orgs = Vec::new();
        for org in &mut self.organisms.iter_mut() {
            if org.old_enough() {
                elig_orgs.push(org.clone());
            }
        }
        ...
        if elig_orgs.is_empty() {
            for org in self.organisms.iter().clone() {
                elig_orgs.push(org.clone());
            }
        }

        // Only choose from among the top ranked orgs
        let poolsize = (elig_orgs.len() - 1) as f64 * self.params.survival_threshold();
        ...
        // if (super::rand_float() < *self.params.mutate_only_prob()) || poolsize == 0f64 {
        //
        //     // Choose the random parent
        //     if let Some(mut mom) = self.random_organism() {
        //         let new_genome = mom.genome().duplicate();
        //     }
        //
        ...
        //     				new_genome->mutate_link_weights(params.mut_power(),1.0,GAUSSIAN);
        //     			}
        ...
        //
        //
        //     	// TODO Use a roulette Wheel here
        //
        //     	mom=(*curorg);
        //
        //     	//Choose random dad
